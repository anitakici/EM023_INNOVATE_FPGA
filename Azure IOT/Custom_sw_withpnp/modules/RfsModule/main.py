...
from package.adcsensor import *
from package.waterSensor import *
...
adc = AdcSensor(name="adc", real=False)
water_sensor = waterSensor(name='waterSensor', real=False)
...
async def main():
    try:
        ...
        delay = 10
        watchdog_task  = None
        global isEdge, isReal, control_bridge, data_bridge, gs, rfs, thc, adc
        ...
        if(hostname in 'de10nano'):
            ...
            if ( bridges[0] is not None ) : 
                ...
                rfs = RfsSensor(name='rfsSensors', real=False, offset=0x40100)
                water_sensor = waterSensor(name="waterSensor", real=False, offset=0x40300)
                thc = ThresholdController(real=False,bridge=data_bridge,offset=0x40200)
        ...
        init_patch= {
            'thresholdProperty': {
                ...
                'mz': { 'min': -100, 'max': 100},
                'ph': {'min': 0, 'max':14},
                'phosphorus': {'min': 77, 'max':114},
                'nitrogen': {'min': 33, 'max':514},
            },
            "$version": 1
        }
            ...
            while True:
                try :
                    if(isReal) :
                        ...
                        adc_data = adc.get_telemetries(data_bridge)
                        water_sensor_data = waterSensor.get_telemetries(data_bridge)
                    else : 
                        ...
                        if water_sensor_data["pH"] >6.7 and water_sensor_data["pH"]<6.94:
                            temp = True
                        if water_sensor_data["phosphorus"] > 40:
                            temp = True
                        if water_sensor_data["nitrogen"] > 60:
                            temp = True
                        if temp:
                            water_sensor_data["status"] = "DANGER: MUCILAGE CAN FORM!"
                        else:
                            water_sensor_data["status"] = "Water is clear."
                    msg = water_sensor.create_component_telemetry(water_sensor_data)
